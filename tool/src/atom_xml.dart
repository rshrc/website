...
String generateAtomXml(List<Article> articles) {
  ...
  <entry>
    ...
    <title>${_htmlEntitiesToXml(article.title)}</title>
...
String _htmlEntitiesToXml(String html) {
  ...
  return html.replaceAllMapped(_htmlEntityPattern, (match) {
    var entity = htmlEntities[match[1]];
    if (entity == null) {
      stderr.writeln('Unmatched HTML entity: ${match[0]}');
      exitCode = 1;
    }
    return entity ?? match.input;
  });
}
